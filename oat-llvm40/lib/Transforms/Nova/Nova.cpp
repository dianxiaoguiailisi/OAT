...
bool Nova::runOnModule(Module &M) {
    GlobalStateRef GS;//全局状态
    Function *f = NULL;//存储main函数
...
Function *Nova::ResolveCall(GlobalStateRef gs, CallInst &I) {
    // 只处理了直接调用，而对间接调用（indirect call）的处理需要在后续添加。
    if ((*(gs->vMap))[&I])//如果该调用指令处理过
        return NULL;
    else {
        // 标记已经访问过
        (*(gs->vMap))[&I] = true;
...
void Nova::VisitSCC(GlobalStateRef gs, SCC &scc) {/*gs:全局状态；scc：当前的强联通图*/
    ...
    for (SCC::iterator BBI = scc.begin(),
        ...
        for (Instruction &I: *(*BBI)) {
            if (auto *callInst = dyn_cast<CallInst>(&I)) {//是调用指令
