...
bool CollectICallHints::instrumentICall(Instruction *I, int fid, int count) {
    ...
    //在模块M中插入函数声明
    Constant *ConstCollectICallHints= M->getOrInsertFunction("__collect_icall_hints", VoidTy,I64Ty,I64Ty, I64Ty, nullptr);

    Function *FuncCollectICallHints= cast<Function>(ConstCollectICallHints);//类型常量指针（Constant*）转换为 Function* 类型的指针
    //参数：
    constFid = ConstantInt::get((IntegerType*)I64Ty, fid);
    constCount = ConstantInt::get((IntegerType*)I64Ty, count);
    castVal = CastInst::Create(Instruction::PtrToInt/*一种类型转换操作(指针类型转换为整数类型)*/, targetFunc/*待转换的值【目标函数的指针（一个内存地址）】*/,I64Ty,"ptrtoint"/*转换操作的名称*/, I/*插入该指令的位置*/);

    //插入函数调用指令B：用于调用函数 FuncCollectICallHints 并传递参数
    B.CreateCall(FuncCollectICallHints/*目的地址：插桩函数*/, {constFid, constCount, castVal}/*{插桩函数参数}*/);
